...
print()
print('gradient descent with Armijo, initial point=(-1.5, 1)')
...
for const_ss in [0.1, 0.01]:
	...
	print()
	print('gradient descent with constant stepsize, initial point=(-1.5, 1) ', const_ss)
...
print('  solution:', x_traces_armijo[-1])
print('  value:', f_value_armijo)
utils.plot_traces_2d(f_2d, x_traces_armijo, path+'gd_traces_armijo_p1d.pdf')
utils.plot(gap, x_traces_armijo, path+'gd_error_armijo_p1d.pdf')
...
plt.ylabel('stepsize')
plt.tight_layout(pad=0.1)
fig.savefig(path+'gd_armijo_ss_p1d.pdf')


# Problem e
for const_ss in [0.005,0.01,0.1]:
	if const_ss != 0.005:
		maxiter=1
	else:
		maxiter=100000
	x_traces_css = gd.gd_const_ss(fp, x0, stepsize=const_ss, maxiter=maxiter )
	f_value_css= f(x_traces_css[-1])
	print()
	print('gradient descent with constant stepsize, initial point=(1.5, 1)', const_ss)
	print('  number of iterations:', len(x_traces_css)-1)
	...
	utils.plot_traces_2d(f_2d, x_traces_css, path+f'gd_traces_css{const_ss}_p1e.pdf')
	utils.plot(gap, x_traces_css, path+f'gd_error_css{const_ss}_p1e.pdf')
